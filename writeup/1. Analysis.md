...
A highly demanded feature was for a  leaderboard with **60%** of users demanding it. This would not be too hard to implement. 
...
The main gameplayloop of this game is that the player, as Mario, moves between two sides of the screen, across different platforms tyring to empty cement into two trucks. The cement must fall through two sets of hoppers. Mario earns **one** point when Marios allows the cement to fall from the upper hopper and **two** points when it falls from the lower one. However each hopper can only hold up to three loads of cement, any overflow will cause Marios and in turn the player a miss. Otherways Mario can earn a miss are: Getting crushed by an elevator platform, reaching the top and bottom of the elevator. There is a section at the bottom of the lift which the player can use to prvent themselves from falling all the way to the bottom. If a player reaches **300** points without a miss, the points will be worth double until a miss all misses will also be cleared. The game ends when the player reaches 3 misses.
